...
def ingestData(dirpath,storm,filesuffixes):
    ...
    # Create table name from storm tack name.
    storm = storm.lower()
    ...
    if it does not exist. The csvswan63 directory will temporarily store csv
    ...
    """
    os.chdir(dirpath+'nc')
    if len([f for f in glob.glob("csvswan63")]) == 0:
        os.mkdir("csvswan63")

    # Get input netcdf file name using storm track name, and file suffixes for each of the swan63 variables. 
    ...
    with xr.open_dataset(inncs[0]) as nc0:
        ...
        for i in range(ntime):
            ...
            try:
                # get hs, tps, and dir variables for specific timestamp.
            ...
            # Convert minimum value of hs, tps, and dir data to NaN.
            ...
            df.to_csv('csvswan63/'+outcsvfile, encoding='utf-8', header=True, index=False)
            ...
            stream = os.popen('timescaledb-parallel-copy --db-name reg3sim --connection "host=localhost user=data password=adcirc sslmode=disable" --table '+tablename+' --file '+'csvswan63/'+outcsvfile+' --skip-header --workers 4 --copy-options "CSV"')
            output = stream.read()

            # Remove the csv file, after the data has been copied to database.
            os.remove('csvswan63/'+outcsvfile)
...
# Define file suffixes for each of the swan63 variables.
filesuffixes = ['_swan_HS.63_mod.nc','_swan_TPS.63_mod.nc','_swan_DIR.63_mod.nc']
# Ingest data into database from netcdf files.
